...
def main():
    board = Board()  # Instancia um objeto da classe Board
    current_player = 1
    iterations = 1000

    while True:
        board.print_grid()  # Chama o método print_grid() do objeto board
        if current_player == 1:
            print("Player 1's turn")
            col = int(input("Enter the column to drop your piece: "))
            if board.Grid[0][col] != 'X':  # Acessa o atributo Grid do objeto board
                print("Column is full. Try again.")
                continue
            row = Make_Move(board, col, 'R')  # Passa o objeto board para a função Make_Move
            board.Grid[row][col] = 'R'  # Acessa o atributo Grid do objeto board
        else:
            print("Player 2's turn (AI)")
            best_move = get_best_move(board.Grid, iterations)  # Acessa o método get_best_move() com o atributo Grid do objeto board
            if best_move == -1:
                print("No valid moves for AI. Try again.")
                continue
            row = Make_Move(board, best_move, 'B')  # Passa o objeto board para a função Make_Move
            board.Grid[row][best_move] = 'B'  # Acessa o atributo Grid do objeto board

        if Game_is_Over(board, current_player):  # Passa o objeto board para a função Game_is_Over()
            board.print_grid()  # Chama o método print_grid() do objeto board
            print(f"Player {current_player} wins!")
            break

        if len([cell for row in board.Grid for cell in row if cell == 'X']) == 0:  # Acessa o atributo Grid do objeto board
            board.print_grid()  # Chama o método print_grid() do objeto board
            print("It's a draw!")
            break
